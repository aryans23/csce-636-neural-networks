...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    """
    ### YOUR CODE HERE
    embedding = tf.get_variable("word_embeddings", [len(self.vocab), self.config.embed_size])
    batch_size = self.config.batch_size
    # num_batches = int(self.encoded_train.shape[0]/batch_size)
    new_shape = int(self.encoded_train.shape[0]/batch_size) * batch_size
    embedded_word_ids = tf.nn.embedding_lookup(embedding, self.encoded_train[:new_shape])
    print("embedded_word_ids.shape = ",embedded_word_ids.shape)
    inputs = [embedded_word_ids[x:x+batch_size] for x in range(0, int(embedded_word_ids.shape[0]), batch_size)]
...
----------------------------------------------------------------------------------------------------
    ...
    losses = [tf.contrib.legacy_seq2seq.sequence_loss(logits=opt,targets=label) for \
          opt, label in zip(output, labels_as_list)]
